...
class CfpSphinx(object):
    ...
    def __gen_tree_index(self, path, title='', maxdepth=1, members=''):
        ...
        rst = self.tree_templ.format(
            ttitle=title, tmaxdepth=maxdepth, tmembers=members)
    ...
    def __gen_gubernias(self, root_dir):
        ...
        self.__gen_tree_index(self.root_dir, members=_g_list, maxdepth=2)
    ...
    def __gen_uezds(self, g_id, g_name, pdir):
        ...
        self.__gen_tree_index(
            path=pdir, title=self.format_header(g_name), members=_u_list)
    ...
    def __gen_localities(self, u_id, u_name, pdir):
        ...
        self.__gen_tree_index(
            path=pdir, title=self.format_header(u_name), members=_l_list)
    ...
    def __gen_churches(self, l_id, l_name, pdir):
        ...
        self.__gen_tree_index(
            path=pdir, title=self.format_header(l_name), members=_ch_list)

    def __gen_datasheets(self, ch_id, ch_name, pdir):
        self.cur.execute(
            "SELECT * FROM cfp_doc WHERE church_id=?", (ch_id,))
        data = self.cur.fetchall()

        rst = self.datasheet_templ.format(
            ttitle=self.format_header(ch_name), tdatasheet="Data table here!")
